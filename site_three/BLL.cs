...
namespace xinxi
{
    public class BLL
    {
        ...
        public List<htmlPara> GetXML()
        {
            ...
            DataTable dt = SqlHelper.ExecuteDataTable("select top 1000 Id, title,titleURL,addTime from htmlPara order by Id desc");
            if (dt.Rows.Count < 1)
                return null;
        ...
        public List<string> GetUrl(int count)
        {
            ...
            DataTable dt = SqlHelper.ExecuteDataTable("select top "+count+" titleURL from htmlPara order by Id desc");
            if (dt.Rows.Count < 1)
                return null;
            foreach (DataRow row in dt.Rows)
            {
        ...
        {
            htmlPara hPara = new htmlPara();
            DataTable dt = SqlHelper.ExecuteDataTable(@"select * from htmlPara h left join columnInfo c on h.columnId=c.Id where columnId =@columnId and h.Id=@Id",
        ...
        public List<htmlPara> GetHtmlBAPage(string columnId, string Id)
        {
            ...
            DataTable dt = SqlHelper.ExecuteDataTable(@"select top 1 -1 as [Sort],Id,title,columnId,titleURL from (select top 1 Id,title,columnId,titleURL from htmlPara where columnId=@columnId and Id<@Id order by Id desc) as tab_up 
        ...
        {
            List<htmlPara> hList = new List<htmlPara>();
            DataTable dt = SqlHelper.ExecuteDataTable(@"select top 10 Id,title,titleURL,titleImg from htmlPara where userId=@userId and columnId!=@columnId order by addTime desc",
        ...
        {
            List<htmlPara> hList = new List<htmlPara>();
            DataTable dt = SqlHelper.ExecuteDataTable(@"select top 10 Id,title,titleURL,titleImg,addTime from htmlPara where userId=@userId and columnId=@columnId order by addTime desc",
               new SqlParameter("@userId", userId),
               new SqlParameter("@columnId", cId));
            if (dt.Rows.Count < 1)
                return null;
        ...
        public List<htmlPara> GetHtmlList(string columnId, int pageIndex, int pageSize)
        {
            //分页查询
            List<htmlPara> hList = new List<htmlPara>();
            DataTable dt = SqlHelper.ExecuteDataTable(@"select Id,title,titleURL,titleImg,userId,columnId,articlecontent,city,smallCount,companyName,ten_qq,com_web,addTime from 
        ...
        {
            List<htmlPara> hList = new List<htmlPara>();
            DataTable dt = SqlHelper.ExecuteDataTable(@"select top (CONVERT(int,@count)) Id,title,titleURL,addTime from htmlPara where columnId !='@columnId' order by addTime desc",
               new SqlParameter("@columnId", columnId),
...
where RANK2<=10").Tables[0];
        ...
        {
            int a = SqlHelper.ExecuteNonQuery(@"INSERT INTO htmlPara
         ([title]
           ,[titleURL]
           ,[articlecontent]
           ,[columnId]
     ...
     VALUES
        ...
        {
            object tUrl = SqlHelper.ExecuteScalar("select top 1 titleURL from htmlPara where userId=@userId order by Id desc",
        ...
        public int GetMaxId()
        {
            ...
            try
            {
                ob = SqlHelper.ExecuteScalar("select Id  from htmlPara order by Id desc");
                if (ob == null)
                    ob = 0;
            }
            catch (Exception ex)
        ...
        {
            return (int)SqlHelper.ExecuteScalar("select count(*)  from htmlPara where columnId=@columnId",
                new SqlParameter("@columnId", columnId));
        }
        /// <summary>
        /// 获取栏目信息
        ...
        public List<columnInfo> GetColumns(string sqlstr)
        {
            ...
            DataTable dt = SqlHelper.ExecuteDataTable("select * from columnInfo " + sqlstr);
            if (dt.Rows.Count < 1)
                return null;
            foreach (DataRow row in dt.Rows)
            {
        ...
        {
            List<htmlPara> hList = new List<htmlPara>();
            DataTable dt = SqlHelper.ExecuteDataTable(@"SELECT TOP 10 Id,title,titleURL,titleImg,addTime FROM htmlPara where userId=@userId and DateDiff(dd,addTime,getdate())<=7 order by  newid()",
               new SqlParameter("@userId", userId));
            if (dt.Rows.Count < 1)
                return null;
            foreach (DataRow row in dt.Rows)
